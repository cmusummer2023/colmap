...
parser.add_argument('--src',type=str,help='path to input images folders',required=True)
...
subprocess.run(["colmap", "model_converter",
                "--input_path", "colmap/results/sparse/0",
                "--output_path", "colmap/results/txts",
                "--output_type", "TXT"])

# get transform.json
subprocess.run(["python", "instant-ngp/scripts/colmap2nerf.py",
                "--images", "colmap/images",
                "--text", "colmap/results/txts",
                "--out", "colmap/original.json"])
